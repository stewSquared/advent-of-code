...
@main def ackrun(): Unit =
  ...
  def ackLit(m: Lit, n: Lit, r8: Lit): Lit = (m, n) match
  ...
  def ackOpt(m: Lit, n: Lit, r8: Lit): Lit =
    inline def calc = (m, n) match
      ...
      case (m, n) if n == 0.toLit => ackOpt(m + 0x7FFF.toLit, r8, r8)
      case (m, n)                 => ackOpt(m + 0x7FFF.toLit, ackOpt(m, n + 0x7FFF.toLit, r8), r8)
    memo.getOrElseUpdate((m,n), calc)

  // println:
  //   (0 to 5).map(n => ackLit(3.toLit,n.toLit,1.toLit)).mkString(" ")

  // println:
  //   (0 to 5).map(n => ackOpt(3.toLit,n.toLit,1.toLit)).mkString(" ")
  ...
  // Found r8=25734 giving ack(4,1)=6
